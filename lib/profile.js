 ...
 */
export function chartPreviousClose(chart) {
...
/**
 * A quote-shaped object built from a chart, for when the live quote is missing.
 *
 * The wallet needs this whenever the session-gated quotes endpoint is
 * unavailable, which on a rate-limited host is routine. It used to build its
 * own, reading `chart.previousClose` directly, and that is the close before the
 * *whole range*: on a MAX chart Microsoft's day change came out at +541,057%
 * because it was comparing today against 1986.
 *
 * The same bug, in the same field, that `chartPreviousClose` was written to fix
 * for the company view. It existed twice because the logic lived in a private
 * function and the second caller could not reach it. Now there is one.
 *
 * Where no honest previous close exists, on weekly or monthly bars, the change
 * is null and the table shows nothing rather than something wrong.
 */
export function quoteFromChart(chart, stored = null) {
  if (!chart) return null;

  /*
   * The stored close wins, because a day change is not a property of the chart.
   *
   * Deriving it from the chart makes it depend on the range the reader happens
   * to be looking at, which is how MAX first produced +541,057% and then, once
   * that was refused, produced nothing at all on every range coarser than
   * daily. Both are wrong in the same way: yesterday's close is a fact, and it
   * does not stop existing because the chart is showing twenty years.
   *
   * The warehouse holds daily closes for every tracked company, so it can
   * answer whatever the chart is doing. It is as of the last completed session,
   * which on a weekday afternoon is yesterday rather than this second, and that
   * is the trade being made: a real number a few hours old, over a live one
   * that is missing.
   *
   * The chart is the fallback's fallback, for anything the warehouse does not
   * track, and there it stays range-aware and yields nothing rather than
   * nonsense.
   */
  const previousClose = stored?.prev_close ?? chartPreviousClose(chart);
  ...
  return {
    ...
    // Where the comparison came from, so the view can say the move is as of the
    // last close rather than implying it is live.
    previousCloseAsOf: stored?.prev_close != null ? (stored.trade_date ?? null) : null,
  };
}
