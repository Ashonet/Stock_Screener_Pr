 ...
 */
export function trailingYearTotals(payments, windows = 11) {
 ...
 * @param {object} args.valuation       optional context from lib/valuation.js:
 *                                      the multiple read against the company's
 *                                      own past and its peers. Omitted, the
 *                                      valuation pillar falls back to the
 *                                      absolute band alone, which is what the
 *                                      score history does for past periods
 *                                      where no peer group can be reconstructed.
 */
export function buildScore({ summary = {}, financials = [], dividendPayments = [], valuation = null }) {
  ...
  // Both are null when the warehouse could not supply enough history or a big
  // enough peer group; `pillar` renormalises over what has a score, so the
  // pillar falls back toward the absolute band rather than losing its value.
  const own = valuation?.own ?? null;
  const peerContext = valuation?.peers ?? null;
  ...
  const pillars = [
    ...
    // Valuation is read three ways, because no one of them is sufficient.
    //
    // The absolute band is the weakest at judging whether a given multiple is
    // deserved (a utility and a software company on 22 are not equally priced)
    // and the only one that can see a company and its whole industry inflate
    // together, which both relative readings are blind to by construction. So
    // it keeps a quarter of the weight rather than the 0.6 it once held alone.
    pillar('Valuation', reit ? 'Priced on FFO, not earnings' : 'Priced on earnings', [
      reit
        ...
        : { label: 'P / E (TTM)', display: mult(trailingPE), score: scoreBetween(trailingPE, 35, 12), weight: 0.25 },
      {
        label: reit ? 'P / FFO vs its own history' : 'P / E vs its own history',
        display: own == null ? null : `${own.ratio.toFixed(2)}×`,
        score: own == null ? null : scoreBetween(own.ratio, 1.3, 0.7),
        weight: 0.3,
        hint:
          own == null
            ? null
            : `${own.current.toFixed(1)}× against a ${own.median.toFixed(1)}× median over ${own.observations} months. ` +
              'Below 1.0× is cheaper than this company has typically been, which is not the same as cheap: a multiple often compresses because earnings are expected to fall.',
      },
      {
        label: reit ? 'P / FFO vs peers' : 'P / E vs peers',
        display: peerContext == null ? null : `${peerContext.ratio.toFixed(2)}×`,
        score: peerContext == null ? null : scoreBetween(peerContext.ratio, 1.3, 0.7),
        weight: 0.25,
        hint:
          peerContext == null
            ? null
            : `${peerContext.current.toFixed(1)}× against a ${peerContext.median.toFixed(1)}× median across ${peerContext.count} ${peerContext.basis} peers${peerContext.group ? ` (${peerContext.group})` : ''}. ` +
              'Below 1.0× is cheaper than the group, which the history comparison cannot see when a whole industry rerates.',
      },
      yieldHistoryUsable
        ? {
