 ...
 */
export async function marketMap() {
  const rows = await query(`
    with ranked as (
      select
 ...
 */
export async function scoringInputs() {
  const [securities, financials, dividends] = await Promise.all([
    query('select symbol, name, industry, is_reit from dim_security'),
    query(`select * from fct_financials where period_type = 'annual' order by symbol, period_end`),
    query('select symbol, pay_date, amount from fct_dividends order by symbol, pay_date'),
  ]);
