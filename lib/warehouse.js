...
/*
 * Sized for the container this runs in, not for the machine underneath it.
 *
 * DuckDB sizes its buffer pool at roughly 80% of detected RAM and takes a
 * thread per core, and inside a container it reads the host's figures rather
 * than the cgroup limit. Left alone on the 512MB instance this deploys to, it
 * concluded it had 25GB and two dozen threads.
 *
 * That was survivable while the warehouse was 84MB, because there was never
 * enough data to fill a pool that big. Loading the full universe took the file
 * past 700MB and it stopped being survivable: one detail-view request pulled
 * the process to 721MB, the container was killed for it, and the platform
 * answered the next request with a 503 whose body is an HTML page — which the
 * client reports as the bare "Server returned 503", having failed to parse it.
 *
 * The cap leaves Node its own headroom inside the instance. DuckDB spills to
 * `temp_directory` rather than failing when a query genuinely needs more, so
 * this trades a little speed on the heaviest queries for not being killed.
 * Both are raisable by environment variable where there is more room.
 */
const MEMORY_LIMIT = process.env.WAREHOUSE_MEMORY_LIMIT || '256MB';
const THREADS = process.env.WAREHOUSE_THREADS || '2';
...
function open() {
  ...
  try {
    ...
    db = new duckdb.Database(DB_PATH, {
      ...
      threads: THREADS,
      // Read-only means DuckDB cannot spill beside the database file, so it
      // needs somewhere it is allowed to write or a query that outgrows the
      // cap fails instead of going to disk.
      temp_directory: join(tmpdir(), 'duckdb-warehouse'),
    });
    status = 'ready';
    console.log(`[warehouse] opened ${DB_PATH} (memory_limit ${MEMORY_LIMIT}, threads ${THREADS})`);
  } catch (err) {
    status = 'unavailable';
...
/**
 * Quote rows built from stored bars, shaped like the live ones.
 *
 * The watchlist and the market strip both "fell back" from the session-gated
 * quote endpoint to chart metadata, which needs no crumb — but that is the same
 * host behind the same IP. When the upstream is limiting the address rather
 * than the endpoint, both legs fail, every promise is filtered out and the
 * route answers 200 with an empty list. Nothing throws, so the header reports
 * "Updated" over a column of dashes: the worst of both, a page that is wrong
 * and confident.
 *
 * Storage is the leg that does not share a failure mode with the other two.
 * Symbols it has never heard of are simply absent from the result, so the
 * caller can tell what it managed to price and what it did not.
 */
export async function quoteSnapshot(symbols) {
  const list = [...new Set(symbols)].filter(Boolean);
  ...
  const rows = await query(
    `with latest as (
     ...
     select
       s.symbol, s.name, s.currency, s.exchange, s.quote_type, s.ingested_at,
       l.trade_date, l.close, l.prev_close, l.volume,
       w.high_52w, w.low_52w
     from dim_security s
     join latest l on l.symbol = s.symbol and l.rn = 1
     left join window52 w on w.symbol = s.symbol
  ...
  return plain(rows).map((row) => {
    ...
    const previousClose = row.prev_close ?? null;
    ...
    return {
      ...
      quoteType: row.quote_type ?? 'EQUITY',
      price,
      previousClose,
      change,
      ...
      marketTime: row.trade_date ? Date.parse(`${row.trade_date}T00:00:00Z`) : null,
      // Marks the row as stored so the header can say so; a live quote has no
      // such field, which is what makes its absence mean "live".
      stored: true,
      storedAsOf: row.ingested_at ? new Date(row.ingested_at).toISOString() : null,
    };
  });
}
 ...
 */
export async function scoringInputs({ index = STUDY_INDEX, periodType = 'annual' } = {}) {
  /*
   * `index: null` widens this to every scored company rather than one index,
   * and `periodType` chooses the statements behind the score.
   *
   * Both exist for the reconstructed side of the movers view. An annual score
   * steps once a year, so over three months it finds eleven companies moved
   * where the quarterly reconstruction finds two hundred and ninety-one; and
   * the movers view covers the whole universe, not the study index. The
   * defaults are the grade study's, which is the older caller.
   */
  const member = index
    ? `symbol in (select symbol from dim_index_membership where index_name = ? and is_current)`
    : `symbol in (select symbol from mart_quality_score)`;
  const args = index ? [index] : [];
  const period = periodType === 'quarterly' ? 'quarterly' : 'annual';

  const [securities, financials, dividends] = await Promise.all([
    query(`select symbol, name, sector, industry, is_reit from dim_security where ${member}`, args),
    query(
      `select * from fct_financials
       where period_type = '${period}' and ${member}
...
/** Every date a score snapshot was written, oldest first. */
export async function scoreSnapshotDates() {
  const rows = await query('select distinct score_date from fct_score_history order by score_date');
  return plain(rows).map((row) => row.score_date);
}

/**
 * Two score snapshots joined, so the difference between them can be read.
 *
 * Left joined from the newer date, never inner: a company absent from the older
 * snapshot has to arrive with a null previous score so the caller can tell
 * "was not covered yet" from "did not move". Inner joining would silently drop
 * the 1,467 companies that joined the universe in August, which is the same
 * error as calling them movers, just quieter.
 *
 * Name and sector come from the score mart rather than the history, which
 * carries neither.
 */
export async function scoresBetween(fromDate, toDate) {
  const rows = await query(
    `select
       now.symbol,
       m.name,
       m.sector,
       m.grade,
       now.overall_score            as score,
       now.coverage_pct,
       past.overall_score           as previous_score,
       past.grade                   as previous_grade
     from fct_score_history now
     left join fct_score_history past
       on past.symbol = now.symbol and past.score_date = ?
     left join mart_quality_score m on m.symbol = now.symbol
     where now.score_date = ?`,
    [fromDate, toDate],
  );
  ...
  return plain(rows).map((row) => ({
    ...
    change: row.previous_score == null ? null : row.score - row.previous_score,
    gradeChanged: row.previous_grade != null && row.previous_grade !== row.grade,
  }));
}

/** The newest snapshot alone, for a range with nothing behind it to compare. */
export async function scoresAt(date) {
  const rows = await query(
    `select now.symbol, m.name, m.sector, now.grade, now.overall_score as score, now.coverage_pct
     from fct_score_history now
     left join mart_quality_score m on m.symbol = now.symbol
     where now.score_date = ?`,
    [date],
...
/**
 * The earliest period end on record for each statement frequency.
 *
 * Read before building a timeline rather than after: reconstructing the whole
 * universe costs a couple of seconds, and there is no point paying it to
 * discover that quarterly statements do not reach 2023.
 */
export async function statementCoverage() {
  const rows = await query(`
    select period_type, cast(min(period_end) as varchar) as oldest, cast(max(period_end) as varchar) as newest
    from fct_financials
    where symbol in (select symbol from mart_quality_score)
    group by period_type`);

  const out = {};
  for (const row of plain(rows)) {
    out[row.period_type] = { oldest: row.oldest?.slice(0, 10) ?? null, newest: row.newest?.slice(0, 10) ?? null };
  }
  return out;
}

/**
 * What each share price did between two dates.
 *
 * Sits beside the score change in the movers table, because "its grade fell
 * seven points" and "the shares rose forty percent" are the two halves of the
 * same question, and reading either alone is how you draw the wrong conclusion
 * from it.
 *
 * The unadjusted close, so this is price movement rather than total return: the
 * dividend is not what the reader is asking about here, and the score beside it
 * already accounts for yield separately.
 *
 * ## Splits
 *
 * A split the stored close was never restated for leaves a step in the series,
 * and a change measured across that step is measuring the split. Porch Group
 * appears to have risen two thousand percent over three years; it actually did,
 * but Vertical Bio's twenty-nine-fold "gain" on 31 August is a one-for-twenty-
 * five reverse split and nothing else.
 *
 * The two are told apart with int_split_corrections rather than by the size of
 * the move, because size cannot tell them apart: Amylyx fell 82% in a session
 * in March 2024 when its ALS drug failed, which a magnitude rule flags as a
 * split and suppresses. It is not in the split table at all, and its crash is
 * exactly the kind of thing this column exists to show.
 *
 * That table already carries `close_step`, the observed jump in close at each
 * split date. It is about 1 for the 1,592 splits the upstream restated properly
 * and materially off 1 for the 85 it did not, so multiplying the opening price
 * by the steps inside the window puts both ends on the same scale. No guessing,
 * and no dashes.
 */
export async function priceChangeBetween(fromDate, toDate) {
  if (!fromDate || !toDate) return new Map();

  const rows = await query(
    `with opened as (
       select symbol, close, trade_date, row_number() over (partition by symbol order by trade_date desc) as rn
       from fct_prices
       where trade_date <= ?
     ),
     closed as (
     ...
     rescales as (
       -- Every unrestated split inside the window, multiplied together. Steps
       -- near 1 are splits the series already handled and are left out rather
       -- than multiplied in, where rounding noise would accumulate.
       select symbol, exp(sum(ln(close_step))) as rescale
       from int_split_corrections
       where split_date > ? and split_date <= ?
         and close_step is not null
         and (close_step > 1.1 or close_step < 0.9)
       group by symbol
     )
     select
       o.symbol,
       o.close as from_close,
       c.close as to_close,
       cast(o.trade_date as varchar) as from_date,
       cast(c.trade_date as varchar) as to_date,
       coalesce(r.rescale, 1) as rescale
     from opened o
     join closed c on c.symbol = o.symbol and c.rn = 1
     left join rescales r on r.symbol = o.symbol
     where o.rn = 1 and o.close > 0`,
    [fromDate, toDate, fromDate, toDate],
  );

  const out = new Map();
  for (const row of plain(rows)) {
    // The opening price on the closing price's scale.
    const opened = row.from_close * row.rescale;

    /*
     * Both ends landing on the same session means there is no price move to
     * measure, not that the price held still.
     *
     * It happens whenever the scores are fresher than the prices, which is the
     * normal state between a scoring run and the next day's close: on the day
     * this was written the snapshots reached 2 September and most symbols' last
     * bar was the 1st, so every row on the 1D view would otherwise have read a
     * confident 0.0%.
     */
    const measurable = row.from_date !== row.to_date;

    out.set(row.symbol, {
      fromClose: row.from_close,
      toClose: row.to_close,
      fromDate: row.from_date,
      toDate: row.to_date,
      changePct: measurable && opened > 0 ? (row.to_close / opened - 1) * 100 : null,
      // Told apart so the view can say which it is: a price that could not be
      // measured is not a price that did not move.
      unmeasurable: !measurable,
      // Whether a split had to be corrected for, so the view can mark it.
      splitAdjusted: row.rescale !== 1,
    });
  }
  return out;
}
 ...
 */
/**
 * Every tracked company's current earnings multiple, computed once.
 *
 * This is the peer pool, and it takes no symbol: it is the same table whoever
 * is asking. It used to sit inside valuationInputs, which meant opening any
 * ticker rescanned the whole price fact to find each symbol's latest close,
 * joined that to every annual statement and rebuilt the entire market's
 * multiples — to read one row out of it and compare against the rest.
 *
 * It was 417ms of a 530ms ticker view, paid again on every switch, and on the
 * range buttons too, which do not change any of it. Memoised it is paid once.
 *
 * The handle underneath is opened read-only and never reopened while the server
 * runs, so the answer cannot go stale under us; reopen() clears this for the
 * case where that ever changes.
 */
let peerPool = null;

export function peerMultiples() {
  if (peerPool) return peerPool;
  ...
  peerPool = query(
    ...
    .catch((err) => {
      peerPool = null; // let the next caller retry rather than caching the failure
      throw err;
    });
...
export async function valuationInputs(symbol) {
  const [prices, earnings, self, peers] = await Promise.all([
    ...
    // Shared across every ticker, so it is computed once rather than per view.
    peerMultiples(),
  ]);
