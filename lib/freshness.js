 ...
 * to.
 *
 * The chart falls back too, since a rate-limited upstream used to take out the
 * whole ticker rather than one panel, so 'price history' is now one of the
 * kinds that can be served from storage and carries its own ingest time.
 *
...
/**
 * @param {string[]} served  from storage: 'price history', 'fundamentals', 'financials'
 * @param {object}   stored  { pricesAsOf, asOf, financialsAsOf } ingest timestamps
 * @returns {string|null}    ISO timestamp of the most recent relevant ingest
 */
export function storedAsOf(served = [], stored = {}) {
  const candidates = [
    served.includes('price history') ? stored.pricesAsOf : null,
