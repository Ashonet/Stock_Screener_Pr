...
/**
 * The score a company would have carried on a given date.
 *
 * `buildScoreHistory` above answers a different question: it scores each
 * reporting period at that period's own end. That is right for a table of
 * results and wrong for "what changed over the last month", because a score
 * pinned to period ends cannot move between them — every company reads as
 * unchanged over any window containing no earnings date, which is most windows
 * shorter than a quarter. A month of movers came back empty for exactly that
 * reason, and the emptiness was an artefact of where the score was sampled
 * rather than a fact about the market.
 *
 * The live score is not like that. It moves daily, because price feeds the
 * valuation and the yield. So this takes the statements the date is entitled to
 * and prices them *on that date*: the statements step at earnings, the price
 * moves in between, and the difference between two dates is what a reader means
 * by a company's score having moved.
 *
 * Still strictly point-in-time. Only periods that had closed by `date` are
 * used, so nothing is graded on statements published after it.
 */
export function scoreOnDate({
  summary = {},
  financials = [],
  dividendPayments = [],
  closes = [],
  ...
  // The last reporting period that had closed by this date.
  let index = -1;
  for (let i = 0; i < rows.length; i++) {
    if (rows[i].date <= date) index = i;
    else break;
  }
  if (index < 0) return null;

  // Identical to the series buildScoreHistory assembles at that period.
  let series;
  if (quarterly) {
    series = [];
    for (let j = index; j >= 0; j -= 4) {
      series.unshift(trailingTwelveMonths(rows.slice(Math.max(0, j - 3), j + 1)));
    }
    series = series.filter(Boolean);
  } else {
    series = rows.slice(0, index + 1);
  }
  if (!series.length) return null;

  // The price on the date, which is what lets this move between earnings.
  const point = closeAt(sortedCloses, date);
  if (!point) return null;
  const price = point.close;

  const asAt = summaryAsAt(summary, series.at(-1), price);
  const yieldPct = trailingYield(payments, date, price);
  if (yieldPct != null) asAt.summaryDetail.dividendYield = { raw: yieldPct / 100 };

  const score = buildScore({
    summary: asAt,
    financials: series,
    dividendPayments: payments.filter((p) => p.t <= ms(date)),
  });
  if (score?.overall == null) return null;

  return {
    date,
    // Which statements it rests on, so the caller can say how stale they were.
    period: rows[index].date,
    score: score.overall,
    grade: score.grade,
    coverage: score.coverage ?? null,
    price,
