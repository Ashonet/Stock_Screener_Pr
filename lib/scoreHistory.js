...
/**
 * Fold four consecutive quarters into one annual-equivalent row.
 *
 * Returns null unless all four are present: a three-quarter "year" understates
 * every flow by a quarter and would read as a sudden collapse in revenue.
 */
export function trailingTwelveMonths(quarters) {
  if (quarters.length !== 4) return null;

  const row = { date: quarters.at(-1).date };

  for (const field of FLOW_FIELDS) {
    const values = quarters.map((q) => q[field]);
    // A field missing from any quarter cannot be summed into a year without
    // silently reporting a partial total as a whole one.
    row[field] = values.every(isNum) ? values.reduce((a, b) => a + b, 0) : null;
  }
...
export function buildScoreHistory({
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (quarterly) {
      series = [];
      for (let j = i; j >= 3; j -= 4) series.unshift(trailingTwelveMonths(rows.slice(j - 3, j + 1)));
