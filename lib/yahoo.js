...
export async function getDailyBars(symbol, sinceMs, untilMs = Date.now()) {
  ...
  const url =
    `https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(symbol)}` +
    // Splits are requested because the adjusted close cannot be trusted to
    // carry them. Yahoo published Monster's 2:1 on 11 August 2026 in this very
    // field while leaving adjclose unadjusted across it, so the warehouse
    // applies the correction itself and needs the events to do it.
  ...
  // Attached rather than returned separately so the single caller keeps
  // treating the result as the array of bars it has always been.
  bars.splits = Object.values(result?.events?.splits ?? {})
    .map((split) => ({
      t: num(split.date) * 1000,
      numerator: num(split.numerator),
      denominator: num(split.denominator),
    }))
    .filter((split) => split.t > 0 && split.numerator > 0 && split.denominator > 0)
    .sort((a, b) => a.t - b.t);
